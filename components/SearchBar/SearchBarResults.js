...
const SearchBarResults = ({ data, onPlaceSelect }) => {
  const checkIfShouldRedirect = (place) => (e) => {
    // check if we have a onPlaceSelect callback
    // if we do, then we don't want to redirect
    if (onPlaceSelect) {
      e.preventDefault();
      onPlaceSelect(place);
    }
    ...
    <div className="tw-border-t tw-border-slate-300 tw-px-4 tw-py-3">
      {[...(data || [])].map((place) => (
        <Link
          ...
          onClick={checkIfShouldRedirect(place)}
        >
