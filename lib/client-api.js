...
const apiFetch = async (endpointPath, options) => {
  ...
  const apiPrefix =
    process.env.NEXT_PUBLIC_ACTIVAZON_API || "https://activazon.herokuapp.com";
...
export const getCityByRequestIp = async () =>
  apiFetch(`/v2/cities/request_ip`, {
    method: "GET",
  });

export const getCityNearby = async ({ coords }) => {
  const params = new URLSearchParams({});
  if (coords) {
  ...
  return apiFetch(`/v2/cities/nearby/?${params.toString()}`, {
    method: "GET",
  });
};
...
export const deleteSubscription = async (subscription) => {
  try {
    await apiFetch(`/v2/subscriptions/${subscription.id}`, {
      method: "DELETE",
    });
  } catch (e) {
    // ignore
...
export const getAreasNearby = async ({ coords, limit }) => {
  const params = new URLSearchParams({
    limit,
  });
  if (coords) {
  ...
  return apiFetch(`/v2/areas/nearby/?${params.toString()}`, {
    method: "GET",
  });
};
...
export const searchPlaces = async ({ search }) => {
  ...
  return apiFetch(`/v2/search/?${params.toString()}`, {
    method: "GET",
  });
};

// map api
export const searchThisApi = async ({ locationBounds }) => {
  return apiFetch("/v2/map/search-this-area/", {
    method: "POST",
    body: JSON.stringify({ location_bounds: locationBounds }),
    headers: {
      "content-type": "application/json",
    },
  });
